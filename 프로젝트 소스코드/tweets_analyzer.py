...
parser = argparse.ArgumentParser(description="Simple Twitter Profile Analyzer (https://github.com/x0rz/tweets_analyzer) version %s" % __version__,
...
parser.add_argument(
    '-f', '--filter', help='filter by source (ex. -f android will get android tweets only)')
...
def process_tweet(tweet):
    ...
    if tweet.user.utc_offset and not args.no_timezone:
        tw_date = (tweet.created_at +
                   datetime.timedelta(seconds=tweet.user.utc_offset))

    if args.utc_offset:
        tw_date = (tweet.created_at +
                   datetime.timedelta(seconds=args.utc_offset))
...
def get_friends(api, username, limit):
    ...
    friends_lang.clear()
    id_screen_names = {}
    for friend in tqdm_gui(tweepy.Cursor(api.friends, screen_name=username).items(limit), unit="friends", total=limit):
...
def print_stats(dataset, top=5):
    ...
    if sum:
        ...
        # use to adjust column width
        max_len_key = max([len(x) for x in sorted_keys][:top])
...
def print_charts(dataset, title, weekday=False):
    ...
    for key in keys:
        ...
        if args.json is False:
            export_string(
                "%s - %s" % (dataset[key], (int_to_weekday(key) if weekday else key)))
...
def main():
    ...
    if args.save:
        ...
        save_file = open(
            save_path+"/"+now.strftime("%Y-%m-%d_%H-%M-%S")+".json", "w")
        save_file.write("[")
    ...
    if args.utc_offset:
        cprint("[\033[91m!\033[0m] Applying timezone offset %d (--utc-offset)" %
               args.utc_offset)
    ...
    if (end_date - start_date).days != 0:
        ...
        jsono['status_average_tweets_per_day'] = (
            num_tweets / float((end_date - start_date).days))
    ...
    print_charts(activity_weekly,
                 "Weekly activity distribution (per day)", weekday=True)
    ...
    cprint("[+] There are \033[1m%d\033[0m geo enabled tweet(s)" %
           geo_enabled_tweets)
    ...
    if args.friends:
        ...
        cprint("[+] Getting %d @%s's friends data..." %
               (max_friends, args.name))
        '''try:
            get_friends(twitter_api, args.name, limit=max_friends)
        except tweepy.error.TweepError as e:
            if e[0][0]['code'] == 88:
                cprint(
                    "[\033[91m!\033[0m] Rate limit exceeded to get friends data, you should retry in 15 minutes")
    ...
    def out_excelFile():  # 엑셀파일로 데이터 출력
        ...
        if(sum):
            sorted_keys = sorted(
                detected_sources, key=detected_sources.get, reverse=True)
            for k in sorted_keys:
                txt.insert(END, ("- {0}\t\t\t\t{1} ({2}%)").format(
                    k, detected_sources[k], round(((float(detected_sources[k] / sum) * 100)), 2)))
                txt.insert(END, "\n")
